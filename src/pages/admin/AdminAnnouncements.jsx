...
const AdminAnnouncements = () => {
  ...
  const brands = ['DEAR', 'KERALA', 'DIAMOND', 'NAGALAND'];
  const playTypes = ['1D', '2D', '3D', '4D'];

  const currentSlotAnalysis = useMemo(() => {
    // Analytics now filters by Brand, Draw Time AND Play Type for exact matching
    const filtered = purchasedTickets.filter(t => 
      t.title.includes(selectedSlot.draw) && 
      (t.playType === selectedSlot.playType || !t.playType) &&
      t.title.toUpperCase().includes(selectedSlot.brand.toUpperCase())
    );
  ...
  const handleDeclareResult = () => {
    ...
    const resultData = {
      ...
      playType: selectedSlot.playType,
      drawName: selectedSlot.drawName,
      winPositions: winPositions.map(p => ({
        position: p.position,
        number: p.number,
        amount: p.amount
      })),
      number: winPositions[0].number 
    };
    
    addResult(resultData);
    setWinPositions(winPositions.map(p => ({ ...p, number: '' })));
    alert(`Result for ${selectedSlot.brand} ${selectedSlot.playType} (${selectedSlot.drawName}) Broadcasted!`);
          ...
          <div className="border-[1.5px] border-[#ff004d] rounded-[2.5rem] p-8 bg-white shadow-2xl relative overflow-hidden group">
            <div className="flex gap-4 mb-10 border-b border-gray-100 pb-6">
              ...
              <div className="flex-grow">
                ...
                <p className="text-[#ff004d] font-black text-[10px] uppercase tracking-widest leading-none mt-1">Refined Verification Control</p>
              </div>
            </div>
            ...
            <div className="space-y-8">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-[9px] font-black text-gray-400 uppercase tracking-widest ml-1">Lot Name (Brand)</label>
                  <select 
                    value={selectedSlot.brand}
                    onChange={(e) => setSelectedSlot({...selectedSlot, brand: e.target.value})}
                    className="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 h-14 font-black text-[#ff004d] outline-none focus:border-[#ff004d]/30 text-xs"
                  >
                    {brands.map(b => <option key={b} value={b}>{b}</option>)}
                  </select>
                </div>
                <div className="space-y-1.5">
                  <label className="text-[9px] font-black text-gray-400 uppercase tracking-widest ml-1">Play Type</label>
                  <select 
                    value={selectedSlot.playType}
                    onChange={(e) => setSelectedSlot({...selectedSlot, playType: e.target.value})}
                    className="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 h-14 font-black text-gray-700 outline-none focus:border-[#ff004d]/30 text-xs"
                  >
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-[9px] font-black text-gray-400 uppercase tracking-widest ml-1">Slot Time</label>
                  <select 
                ...
                <div className="space-y-1.5">
                  <label className="text-[9px] font-black text-gray-400 uppercase tracking-widest ml-1">Draw Name</label>
                  <input 
                    type="text"
                    value={selectedSlot.drawName}
                    onChange={(e) => setSelectedSlot({...selectedSlot, drawName: e.target.value.toUpperCase()})}
                    className="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 h-14 font-black text-gray-700 outline-none focus:border-[#ff004d]/30 text-[10px]"
              ...
              <div className="space-y-5 pt-4 border-t border-gray-50">
                <div className="flex justify-between items-center px-1">
                  <h3 className="text-[10px] font-black uppercase tracking-widest text-[#ff004d]">Prize Entry: {selectedSlot.playType} Format</h3>
                  <button onClick={handleAddPosition} className="bg-gray-50 p-1.5 rounded-lg text-[#ff004d] hover:bg-red-50">
                    <Plus size={16} />
                  </button>
                </div>
                  ...
                  {winPositions.map((pos, i) => (
                    <div key={i} className="bg-gray-50/50 rounded-2xl p-5 border border-gray-100 space-y-4 shadow-sm group hover:border-[#ff004d]/20 transition-all">
                      <div className="flex justify-between items-center">
                         ...
                         <button onClick={() => setWinPositions(winPositions.filter((_, idx) => idx !== i))} className="text-gray-200 hover:text-red-500">
                            <Trash2 size={14} />
                         </button>
                      </div>
                      ...
                      <div className="flex gap-3">
                         <div className="flex-grow space-y-1.5">
                            <div className="relative">
                               <Hash className="absolute left-4 top-1/2 -translate-y-1/2 text-amber-500 opacity-50" size={14} />
                               <input 
                                 type="text" 
                                 placeholder={`ENTER ${selectedSlot.playType} NO.`} 
                                 ...
                                 maxLength={parseInt(selectedSlot.playType)}
                               />
                            </div>
                         </div>
                         <div className="w-24 space-y-1.5">
                            <div className="relative">
                               <input 
                                 type="number" 
                                 placeholder="₹ Prize" 
                                 className="w-full h-12 bg-white border border-gray-100 rounded-xl px-4 outline-none font-black text-[#ff004d] text-sm shadow-inner"
                                 value={pos.amount}
              ...
              >
                Declare {selectedSlot.playType} Winners <Trophy size={20} />
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-[2.5rem] p-8 border border-gray-100 shadow-xl space-y-8">
            <div className="flex items-center gap-4 border-b border-gray-50 pb-6">
              ...
              <div className="flex-grow">
                <h3 className="text-xl font-black text-gray-900 font-condensed uppercase tracking-tighter italic leading-none">{selectedSlot.brand} - {selectedSlot.playType}</h3>
                <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest leading-none mt-1">Live {selectedSlot.draw} Marketplace Analysis</p>
              </div>
            </div>

            <div className="grid grid-cols-2 gap-4">
              <div className="bg-gray-50 p-6 rounded-[2rem] border border-gray-100 relative overflow-hidden group">
                 <p className="text-[8px] font-black text-gray-400 uppercase tracking-widest mb-1">Total {selectedSlot.playType} Intake</p>
                 <p className="text-2xl font-black text-gray-900 italic tracking-tighter leading-none">{currentSlotAnalysis.totalTickets}</p>
              </div>
              <div className="bg-gray-50 p-6 rounded-[2rem] border border-gray-100 relative overflow-hidden group">
                 <p className="text-[8px] font-black text-gray-400 uppercase tracking-widest mb-1">Unique Combinations</p>
            ...
            <div className="space-y-4">
              <h4 className="text-[10px] font-black uppercase tracking-widest text-[#ff004d] ml-2">Market Load (Most Purchased)</h4>
              <div className="space-y-2">
                {currentSlotAnalysis.topCombinations.length === 0 ? (
                  <p className="text-center py-10 text-[10px] font-black text-gray-300 uppercase italic">No intake for this format yet</p>
                ) : (
                  currentSlotAnalysis.topCombinations.map((item, idx) => (
                    <div key={idx} className="flex items-center justify-between p-4 bg-white border border-gray-100 rounded-2xl shadow-sm">
                       <div className="flex items-center gap-4">
                          ...
                          <div>
                             <p className="text-[8px] font-black text-gray-400 uppercase tracking-widest">Order Count</p>
                             <p className="text-sm font-black text-gray-800">{item.count} Units</p>
                          </div>
                       </div>
                    </div>
                  ))
